   ...
   // m_local_storage = dir.exists();
    ...
    if (aConfig.value("local").toBool()){
        ...
        while (!dir.exists(cfg_pth))
            std::this_thread::sleep_for(std::chrono::milliseconds(1000));
        QFile minio_cfg(cfg_pth);
        minio_cfg.open(QFile::ReadOnly);
        auto cfg = QJsonDocument().fromJson(minio_cfg.readAll()).object();
        auto credential = cfg.value("credential").toObject();*/
        //minio_cfg.close();
    }else{
        m_valid = true;
    ...
    m_ip_port = aConfig.value("ip").toString(rea::GetLocalIP().toString() + ":9000").toStdString(); //"http://192.168.1.122:9000";
    m_access_key = aConfig.value("access").toString("deepsight").toStdString(); //"YKFERVMC3AK54Y1X150B";
    m_secret_key = aConfig.value("secret").toString("deepsight").toStdString(); //"4y0PzVzrvyiE6RzssCbrgO7HCxPIDRrK2pO1qJ5C";
